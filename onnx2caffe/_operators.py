...
def _convert_pool(node, graph, err):
    ...
    elif node.op_type.endswith("AveragePool") or node.op_type.endswith("GlobalAveragePool"):
...
def _convert_pow(node, graph, err):
    input_name = str(node.inputs[0])
    output_name = str(node.outputs[0])
    rate = node.input_tensors[node.inputs[1]] # 几次幂
    layer = myf("Power", node.name, [input_name], [output_name],
                power_param=dict(
                    power=rate)
                )
    # 计算幂次方
